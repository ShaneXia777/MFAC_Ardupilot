...
void AC_PosControl::pso_read(void)
{
        FILE *fp;
        AP_Vehicle::st data;
    ...
    printf("error!\n");
        ...
        fclose(fp);
        XY_eita=data.XY_eita;
        XY_miu=data.XY_miu;
        XY_rou1=data.XY_rou;
        XY_rou2=data.XY_rou;
        XY_rou3=data.XY_rou;
        XY_lamda=data.XY_lamda;
        ...
        T_rou2=data.T_rou;
        T_rou3=data.T_rou;
        T_lamda=data.T_lamda;
    printf("XY_eita=%f, XY_miu=%f, XY_rou=%f,XY_lamda=%f,T_eita=%f, T_miu=%f,T_rou=%f,T_lamda=%f \n",XY_eita, XY_miu,XY_rou1,XY_lamda,T_eita,T_miu,T_rou1,T_lamda);
        return;
}
...
=======
    ...
    //*************************************************************************************************
    // float ux_MFAC=MFAC(curr_vel.x,_vel_target.x,XY_eita,XY_miu,XY_rou1,XY_rou2,XY_rou3,XY_lamda);
    // float uy_MFAC=MFAC(curr_vel.y,_vel_target.y,XY_eita,XY_miu,XY_rou1,XY_rou2,XY_rou3,XY_lamda);
    // accel_target.x+=ux_MFAC;
    // accel_target.y+=uy_MFAC;
...
>>>>>>> 4966fcd133 (first commit)
  ...
  if (now>45000)
{
    // sum1+=0.0016*abs(accel_target.x)+0.5*abs(_vel_target.x-curr_vel.x);
    // sum2+=0.03*abs(accel_target.y)+6*abs(_vel_target.y-curr_vel.y);
    //0916
    sum1+=0.1*abs(accel_target.x)+19.48194*abs(_vel_target.x-curr_vel.x);
    sum2+=0.1*abs(accel_target.y)+21.79665*abs(_vel_target.y-curr_vel.y);
    
// if (now<85000)
//  {
//     ofstream outfile;
// 	outfile.open("XYController1007.csv", ios::out | ios::app);
// 	outfile<<accel_target.x <<","<< accel_target.y<<","<<_vel_target.x <<","<< curr_vel.x<<","<<_vel_target.y <<","<< curr_vel.y<<","<<_pos_target.x<<","<<curr_pos.x<<","<<_pos_target.y<<","<<curr_pos.y<<endl;
// 	outfile.close();
//  }
     if (now>88000 && 0 == num1 && 0==num2)
 	{
		cost1 = sum1/(now-45000);
        cost2 = sum2/(now-45000);
		num1++;
		num2++;
//********************************************************************20220730******************    
        // printf("X Cost is %f\n",cost1/1.177723714);
		// AP::vehicle()->update_pso_cost(cost1/1.177723714,1);
        // printf("Y Cost is %f\n",cost2/1.195701857);
		// AP::vehicle()->update_pso_cost(cost2/1.195701857,2);  
        //1007
        printf("X Cost is %f\n",cost1/1.473985716);
		AP::vehicle()->update_pso_cost(cost1/1.473985716,1);
        printf("Y Cost is %f\n",cost2/1.917629612);
		AP::vehicle()->update_pso_cost(cost2/1.917629612,2);  
	}
 }
}
...
=======
        ...
        //**************************************************
        // float ut_MFAC=MFAC(z_accel_meas,_accel_target.z,T_eita,T_miu,T_rou1,T_rou2,T_rou3,T_lamda);
        // thr_out+=ut_MFAC;
        //*********************************************************
>>>>>>> 4966fcd133 (first commit)
...
if (now>45000)
{
    //sum6+=0.25*abs(thr_out)+0.1*abs(_accel_target.z-z_accel_meas);
    //0916
    sum6+=0.257655*abs(thr_out)+0.1*abs(_accel_target.z-z_accel_meas);
    
    // if (now<85000)
    // {
    // ofstream outfile1;
	// outfile1.open("ThrData1007.csv", ios::out | ios::app);
    // outfile1<<thr_out<<","<<_accel_target.z<<","<<z_accel_meas<<","<<pos_target_zf<<","<< _inav.get_position_z_up_cm()<<endl;
	// outfile1.close();
    // }

	if (now>88000 && 0 == num6)
	{
        cost6 = sum6/(now-45000);
        num6++;
        //************************************************************************************************************************
        // printf("Throttle Cost is %f\n",cost6/1.168844429);
		// AP::vehicle()->update_pso_cost(cost6/1.168844429,6);
        //1007
        printf("Throttle Cost is %f\n",cost6/1.163533946);
		AP::vehicle()->update_pso_cost(cost6/1.163533946,6);
	}
}
...
//****************************************************************************************************************************
float AC_PosControl::MFAC(float achieved_rate,float desired_rate,float eita,float miu,float rou1,float rou2,float rou3,float lamda)
{
