...
//*************************************************************************************************
void AP_Vehicle::update_pso_cost(float cost,uint8_t type)
{
    FILE *fp;
    if(1==type)
    {
        fp=fopen("X_cost.txt","w+");
        if(!fp)
        {
            printf("update_pso_cost open X_cost.txt error!\n");
            exit(-1);
        }
    }
    else if(2==type)
    {
        fp=fopen("Y_cost.txt","w+");
        if(!fp)
        {
            printf("update_pso_cost open Y_cost.txt error!\n");
            exit(-1);
        }
    }
    else if(3==type)
    {
        fp=fopen("Roll_cost.txt","w+");
        if(!fp)
        {
            printf("update_pso_cost open Roll_cost.txt error!\n");
            exit(-1);
        }
    }
    else if(4==type)
    {
         fp=fopen("Pitch_cost.txt","w+");
        if(!fp)
        {
            printf("update_pso_cost open Pitch_cost.txt error!\n");
            exit(-1);
        }
    }
    else if(5==type)
    {
        ...
        fp=fopen("Yaw_cost.txt","w+");
        if(!fp)
        {
            printf("update_pso_cost open Yaw_cost.txt error!\n");
            exit(-1);
        }
    }
    else if(6==type)
    {
        fp=fopen("Thr_cost.txt","w+");
        if(!fp)
        {
            printf("update_pso_cost open Thr_cost.txt error!\n");
            exit(-1);
        }
    }
    else
    {
        printf("update_pso_cost type error %d error!\n",type);
        return;
    }
    fwrite(&cost,sizeof(cost),1,fp);
    //rewind(fp);
    fclose(fp);
